...
parser.add_argument('--dataset', type=str, default='Osaka', choices=["Shanghai", "New_York", "Los_Angeles","Hongkong","Tokyo","Seoul","Chicago","Osaka"],
...
# Debug: Print the structure of loaded data
print("\n=== Debug: Data Structure ===")
print(f"Type of train_data: {type(train_data)}")
print(f"Keys in train_data: {train_data.files if hasattr(train_data, 'files') else 'No files attribute'}")
print(f"Shape of train_data: {train_data.shape if hasattr(train_data, 'shape') else 'No shape'}")
print(f"First element type: {type(train_data['data'][0]) if 'data' in train_data.files else 'No data key'}")

# Try to access the actual data
if 'data' in train_data.files:
    print(f"train_data['data'] shape: {train_data['data'].shape}")
    print(f"First element of train_data['data']: {train_data['data'][0]}")
    print(f"Type of first element: {type(train_data['data'][0])}")
    if isinstance(train_data['data'][0], np.ndarray):
        print(f"First element is numpy array with shape: {train_data['data'][0].shape}")
        print(f"First element dtype: {train_data['data'][0].dtype}")
    elif isinstance(train_data['data'][0], dict):
        print(f"First element keys: {train_data['data'][0].keys()}")
else:
    print("No 'data' key found in train_data")

# The data might be stored directly as an array
if hasattr(train_data, 'shape') and len(train_data.shape) > 0:
    print(f"train_data has shape: {train_data.shape}")
    print(f"First element: {train_data[0] if len(train_data) > 0 else 'Empty'}")
    print(f"Type of first element: {type(train_data[0])}")

print("\n" + "="*50 + "\n")

# Now try to call get_data_generator with the appropriate data
try:
    ...
    # Alternative: the data might be directly in the npz file as arrays
    # Extract the data arrays
    if 'data' in train_data.files:
        train_data_arr = train_data['data']
        val_data_arr = val_data['data']
        test_data_arr = test_data['data']
        
        # Now try to call get_data_generator with the arrays
        train_data, val_data, test_data, ori_train_data, ori_val_data, ori_test_data = get_data_generator(
            opt, train_data_arr, val_data_arr, test_data_arr, normal=2)
    else:
        raise Exception("Unable to load data. Please check the data format.")
...
if __name__ == '__main__':
    ...
    for epoch in range(2000):
        ...
        for i in range(len(train_data)):
            ...
            inputs = Tensor(node_features).clone()  # (num_nodes, dim_in)
            
            optimizer.zero_grad()
            

            y_pred = model(g, inputs)  # (num_nodes, 2)
